...
final class NetworkServiceTests: XCTestCase {
    ...
    func testFetchProductsSuccess() async throws {
        ...
        let expectedReply = productlist.Reply.Products.ProductsReply(
        ...
        let params = productlist.Requests.Products.ProductsParams(limit: 1, skip: 1)
...
class MockNetworkService: NetworkServicing {
    ...
    func fetchProducts(params: productlist.Requests.Products.ProductsParams) async throws -> productlist.Reply.Products.ProductsReply {
        if let error = mockError { throw error }
        guard let data = mockResponseData else { throw URLError(.badServerResponse) }
        return try JSONDecoder().decode(productlist.Reply.Products.ProductsReply.self, from: data)
    }
